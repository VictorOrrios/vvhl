...
bool Swapchain::initialize(Device& device,VkSurfaceKHR surface,
...
bool Swapchain::createSwapchain(uint32_t width, uint32_t height){
    ...
    // Use concurrent mode to share image betewen families if they are not the same
    ...
    createInfo.preTransform =
        m_swapchainSupport.capabilities.currentTransform;
    ...
    m_config.surfaceFormat = surfaceFormat;
    m_config.presentMode   = presentMode;
...
bool Swapchain::createImageViews(){
    ...
    for (uint32_t i = 0; i < imageCount(); i++){
        ...
        createInfo.format   = m_config.surfaceFormat.format;
        ...
        {
            LOGE("Failed to create swapchain image view nº{}, deleting all stored image views",i);
            destroyImageViews();
            return false;
        }
    }
...
bool Swapchain::recreate(){
    ...
    if (!createSwapchain(m_config.extent.width, m_config.extent.height))
        return false;
