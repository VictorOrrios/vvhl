...
namespace vvhl
{
  ...
  bool GraphicsPipeline::buildShaderStages()
  {
    ...
    {
      // VERTEX
      createShaderStage(VK_SHADER_STAGE_VERTEX_BIT, m_createInfo.rasterShaders.vertex);

      // TESSELATION
      if (m_createInfo.rasterShaders.tessControl.has_value() ^ m_createInfo.rasterShaders.tessEval.has_value())
      {
      ...
      if (m_createInfo.rasterShaders.tessControl.has_value() && m_createInfo.rasterShaders.tessEval.has_value())
      {
        createShaderStage(VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT, m_createInfo.rasterShaders.tessControl.value());
        createShaderStage(VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, m_createInfo.rasterShaders.tessEval.value());
      }

      // GEOMETRY
      if (m_createInfo.rasterShaders.geometry.has_value())
      {
        createShaderStage(VK_SHADER_STAGE_GEOMETRY_BIT, m_createInfo.rasterShaders.geometry.value());
      }

      // FRAGMENT
      createShaderStage(VK_SHADER_STAGE_FRAGMENT_BIT, m_createInfo.rasterShaders.fragment);
    }
    else
    {
      // TASK
      ...
      {
        createShaderStage(VK_SHADER_STAGE_TASK_BIT_EXT, m_createInfo.meshShaders.task.value());
      }

      // MESH
      createShaderStage(VK_SHADER_STAGE_MESH_BIT_EXT, m_createInfo.meshShaders.mesh);
  ...
  bool GraphicsPipeline::createPipeline()
  {
    ...
    {
      LOGE("Dynamic state of graphics pipeline has to have at least the viewport and scissor")
      return false;
    }
    if (m_mode == GraphicsPipelineMode::Raster)
    {
