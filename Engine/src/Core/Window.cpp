...
void Window::setPosition(int x, int y){
    glfwSetWindowPos(m_glfwHandle, x, y);
}
void Window::setFullscreen(bool enabled)
{
    ...
    if (enabled)
    {
        ...
        glfwGetWindowSize(m_glfwHandle, &m_windowedState.width, &m_windowedState.height);
...
void Window::WindowCloseCallback(GLFWwindow* window){
    ...
    self->m_dispatcher.dispatch(
        vvhl::WindowCloseEvent(self));
}

void Window::WindowFocusCallback(
    GLFWwindow* window,
    int focused)
{
    auto* self = getWindow(window);
    ...
    self->m_dispatcher.dispatch(
        vvhl::WindowFocusEvent(
            self,
            focused == GLFW_TRUE));
}

void Window::WindowIconifyCallback(
    GLFWwindow* window,
    int iconified)
{
    auto* self = getWindow(window);
    ...
    self->m_dispatcher.dispatch(
        vvhl::WindowMinimizeEvent(
            self,
            iconified == GLFW_TRUE));
}

void Window::WindowMaximizeCallback(
    GLFWwindow* window,
    int maximized)
{
    auto* self = getWindow(window);
    ...
    self->m_dispatcher.dispatch(
        vvhl::WindowMaximizeEvent(
            self,
            maximized == GLFW_TRUE));
}

void Window::WindowPositionCallback(
    GLFWwindow* window,
    int x,
    int y)
{
