...
bool Input::isKeyPressed(int key) {
  return s_keys[key] != ButtonState::Released;
}

// MOUSE
bool Input::isMouseButtonPressed(int button) {
  return s_mouseButtons[button] != ButtonState::Released;
}
...
// KEYBOARD
void Input::KeyCallback(GLFWwindow *, int key, int, int action, int mods) {
  // Invalid key or non existent dispatcher
  if (key < 0 || !s_dispatcher)
    return;
